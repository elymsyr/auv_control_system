...
NonlinearMPC::NonlinearMPC(const VehicleModel& model, int N, double dt)
    : model_(model), N_(N), dt_(dt), nx_(12), nu_(8), 
      prev_sol_(std::nullopt), obstacle_weight_(0.5), 
      obstacle_epsilon_(0.5), num_obstacles_(5), min_dist_(0.5) {
    setup_optimization();
}
...
MX NonlinearMPC::barrier_function(const MX& position, const MX& obstacles) {
    ...
    for (int i = 0; i < num_obstacles_; ++i) {
        ...
        MX dx = x - obs_x;
        MX dy = y - obs_y;
        MX dist_sq = dx*dx + dy*dy + obstacle_epsilon_;
        MX dist = sqrt(dist_sq);
        
        // Use ONLY ONE of these barrier functions:
        
        // 1. SOFT RECIPROCAL BARRIER (more stable)
        total_penalty += 1.0 / (dist + min_dist_);
        
        // 2. EXPONENTIAL BARRIER (smoother)
        // total_penalty += exp(-dist_sq);
    }
