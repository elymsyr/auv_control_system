...
std::vector<uint8_t> EnvironmentState::serialize() {
    ...
    buffer.reserve(1 + sizeof(VehicleState) + sizeof(VehicleStateDes));

    // Header byte
    buffer.push_back('E');

    // Serialize currentState.front() or zero-fill
    if (!currentState.empty()) {
        auto& s = currentState.front();
        pack_state(s, buffer);
    } else {
        // Append zero bytes for VehicleState
        buffer.insert(buffer.end(), sizeof(VehicleState), uint8_t{0});
    }

    // Serialize desiredState.front() or zero-fill
    if (!desiredState.empty()) {
        auto& d = desiredState.front();
        pack_state(d, buffer);
    } else {
        // Append zero bytes for VehicleStateDes
        buffer.insert(buffer.end(), sizeof(VehicleStateDes), uint8_t{0});
    }
